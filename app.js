...
let selectedSpriteStaticTemp = 'assets/characters/hero-male.png';
let selectedSpriteIdleTemp = 'assets/characters/hero-male-idle.gif';
let isAuthPending = false; // Guard lock to eliminate duplicate toasts/submits
...
// ==========================================
// 2. INPUT SANITIZATION & NOTIFICATIONS
// ==========================================
...
// ==========================================
// 3. UI SCREEN & NAVIGATION CONTROL
// ==========================================
...
// ==========================================
// 4. ANIMATION & AVATAR SYNCHRONIZATION
// ==========================================

/**
 * Renders avatar elements with image preloading and unified timestamp query params
 * to keep base character GIFs and accessory overlays frame-synchronized.
 */
function renderCharacterAvatar(containerId, user) {
  const container = document.getElementById(containerId);
  if (!container || !user) return;

  const baseSprite = user.base_sprite_idle || DEFAULT_USER_PROFILE.base_sprite_idle;
  const equippedList = user.equipped_items || [];

  // Build resolved path list
  const activePaths = [baseSprite];
  equippedList.forEach(itemEntry => {
    if (!itemEntry || itemEntry === 'BASE') return;
    const itemData = ITEM_MANIFEST[itemEntry];
    const resolvedPath = itemData ? itemData.path : itemEntry;
    if (resolvedPath && !activePaths.includes(resolvedPath)) {
  ...
  const applyToDOM = () => {
    let layersHTML = `<img src="${baseSprite}?t=${syncToken}" alt="Base Sprite" class="character-img base-layer" />`;
    
    activePaths.slice(1).forEach(path => {
      layersHTML += `<img src="${path}?t=${syncToken}" alt="Costume Overlay" class="character-img costume-overlay-layer" />`;
    });

    container.innerHTML = layersHTML;
  };

  // Preload frames into memory first so browser paints frame 0 simultaneously
  activePaths.forEach(path => {
    const img = new Image();
    img.onload = img.onerror = () => {
      loadedCount++;
      if (loadedCount >= activePaths.length) applyToDOM();
    };
    img.src = `${path}?t=${syncToken}`;
...
async function handleLocalSignup(e) {
  ...
  try {
    ...
    // Integrate Supabase Client if present
    if (typeof supabaseClient !== 'undefined' && supabaseClient) {
      const { data, error } = await supabaseClient.auth.signUp({
        email: email,
        password: pass,
        options: { data: { trainer_name: newUser.trainer_name } }
      });
      ...
      if (data?.user) newUser.id = data.user.id;
    }
...
async function handleLocalLogin(e) {
  ...
  try {
    ...
    // Handle Supabase Authentication path
    if (typeof supabaseClient !== 'undefined' && supabaseClient) {
      ...
      currentUser = {
        id: user.id,
        email: user.email,
        trainer_name: profile?.trainer_name || user.user_metadata?.trainer_name || DEFAULT_USER_PROFILE.trainer_name,
        wage: profile?.wage ?? DEFAULT_USER_PROFILE.wage,
        food_price: profile?.food_price ?? DEFAULT_USER_PROFILE.food_price,
        event_price: profile?.event_price ?? DEFAULT_USER_PROFILE.event_price,
        lvl: profile?.level ?? DEFAULT_USER_PROFILE.lvl,
        xp: profile?.exp ?? DEFAULT_USER_PROFILE.xp,
        saved_total: profile?.total_saved ?? DEFAULT_USER_PROFILE.saved_total,
        base_sprite_static: profile?.base_sprite_static || DEFAULT_USER_PROFILE.base_sprite_static,
        base_sprite_idle: profile?.base_sprite_idle || DEFAULT_USER_PROFILE.base_sprite_idle,
        equipped_items: profile?.equipped_items || DEFAULT_USER_PROFILE.equipped_items,
        inventory: profile?.inventory || DEFAULT_USER_PROFILE.inventory,
        logs: profile?.logs || DEFAULT_USER_PROFILE.logs,
...
function saveFinancialProfile(e) {
  ...
  let wage = parseFloat(document.getElementById('survey-wage')?.value) || DEFAULT_USER_PROFILE.wage;
  let food = parseFloat(document.getElementById('survey-food')?.value) || DEFAULT_USER_PROFILE.food_price;
  let eventVal = parseFloat(document.getElementById('survey-event')?.value) || DEFAULT_USER_PROFILE.event_price;
...
function loadTrainerSession() {
  ...
  const setNames = document.getElementById('settings-name');
  const setWage = document.getElementById('settings-wage');
  const setFood = document.getElementById('settings-food');
  const setEvent = document.getElementById('settings-event');

  if (setNames) setNames.value = currentUser.trainer_name || '';
  if (setWage) setWage.value = currentUser.wage ?? 15.00;
  if (setFood) setFood.value = currentUser.food_price ?? 7.50;
  if (setEvent) setEvent.value = currentUser.event_price ?? 25.00;
...
// ==========================================
// 6. COMBAT ENGINE & ATTACK MECHANICS
// ==========================================
...
function startBattle() {
  ...
  if (enemyNameEl) enemyNameEl.textContent = monster.name;
  if (enemyContainer) {
    enemyContainer.innerHTML = `<img src="${monster.sprite}?t=${Date.now()}" alt="${monster.name}" class="character-img" />`;
  }
  ...
  setBattleBackgroundByCategory(category);
  updateHPUI();
  setDialogue(`A wild ${monster.name} appears!`);
  showAttackMenu();
  showScreen('screen-battle');
}
...
function startQuestionFlow(attackType) {
  ...
  container.innerHTML = `
    <div class="input-field">
      <input type="text" id="user-reflection" placeholder="Type your reflection answer..." maxlength="80" autofocus />
    </div>
    <button id="btn-submit-reflection" class="btn btn-primary" onclick="processPlayerAttack('${attackType}')">
      ${isHeal ? 'Rest & Strike' : 'Submit Strike'}
    </button>
...
function processPlayerAttack(attackType) {
  ...
  if (battleState.enemyHP <= 0) {
    setDialogue(`FINAL STRIKE! You defeated ${battleState.monsterName}!`);
    setTimeout(victorySavedMoney, 1200);
  } else {
    setDialogue(`Dealt ${damageDealt} DMG to ${battleState.monsterName}!${healMsg}`);

    setTimeout(() => {
      const chosenAttack = getRandomMonsterAttack();
      battleState.playerHP = Math.max(0, battleState.playerHP - chosenAttack.damage);
      setDialogue(`${battleState.monsterName} used ${chosenAttack.name}! (-${chosenAttack.damage} HP)`);
      updateHPUI();

      setTimeout(() => {
        if (battleState.playerHP <= 0) {
          setDialogue(`${battleState.monsterName} overwhelmed your resolve!`);
          setTimeout(giveInAndSpend, 1200);
        } else {
          battleState.isProcessing = false;
          showAttackMenu();
        }
...
function victorySavedMoney() {
  if (typeof confetti === 'function') confetti({ particleCount: 80, spread: 70 });
  showNotification(`Victory! You saved $${battleState.price.toFixed(2)}.`, "success");

  if (currentUser) {
    currentUser.vault_unlock_time = Date.now() + (15 * 60 * 1000); // 15 Minute Cooldown
  }
...
function startVaultTimer(unlockTimestamp) {
  ...
  function updateDisplay() {
    ...
    const vaultEl = document.getElementById('vault-timer');
    if (vaultEl) {
      vaultEl.textContent = `${mins.toString().padStart(2, '0')}:${secs.toString().padStart(2, '0')}`;
    }
  }
...
// ==========================================
// 8. SHOP & RELIABLE EQUIP/UNEQUIP SYSTEM
// ==========================================
  ...
  if (itemId === 'hat_default') {
    ...
    if (isOwned) {
      ...
      if (index > -1) {
        ...
        // Remove items sharing the same slot
